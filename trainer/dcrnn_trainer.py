...
class DCRNNTrainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, target, adj_mat) in enumerate(self.data_loader.get_iterator()):
            data = torch.FloatTensor(data)
            target = torch.FloatTensor(target)
            label = target[..., :self.model.output_dim]  # (..., 1)  supposed to be numpy array
            data, target = data.to(self.device), target.to(self.device)
            # construct the adj_mat here
            adj_mat = utils.calculate_scaled_laplacian(adj_mat, lambda_max=None)  # scipy coo matrix
            adj_mat = self._build_sparse_matrix(adj_mat)  # to PyTorch sparse tensor
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (data, target, adj_mat) in enumerate(self.valid_data_loader.get_iterator()):
                data = torch.FloatTensor(data)
                target = torch.FloatTensor(target)
                label = target[..., :self.model.output_dim]  # (..., 1)  supposed to be numpy array
                data, target = data.to(self.device), target.to(self.device)
