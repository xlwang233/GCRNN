...
class DCRNNEncoder(BaseModel):
    def __init__(self, input_dim, max_diffusion_step, hid_dim, num_nodes, num_rnn_layers):
        ...
        encoding_cells.append(DCGRUCell(input_dim=input_dim, num_units=hid_dim,
                                        max_diffusion_step=max_diffusion_step, num_nodes=num_nodes))

        # construct multi-layer rnn
        for _ in range(1, num_rnn_layers):
            encoding_cells.append(DCGRUCell(input_dim=hid_dim, num_units=hid_dim,
                                            max_diffusion_step=max_diffusion_step, num_nodes=num_nodes))
        self.encoding_cells = nn.ModuleList(encoding_cells)

    def forward(self, inputs, initial_hidden_state, adj_mat):
        ...
        for i_layer in range(self._num_rnn_layers):
            ...
            for t in range(seq_length):
                _, hidden_state = self.encoding_cells[i_layer](current_inputs[t, ...], hidden_state, adj_mat)  # (50, 207*64)
...
class DCGRUDecoder(BaseModel):
    ...
    def forward(self, inputs, initial_hidden_state, adj_mat, teacher_forcing_ratio=0.5):
        """
        ...
        for t in range(1, seq_length):
            ...
            for i_layer in range(0, self._num_rnn_layers):
                hidden_state = initial_hidden_state[i_layer]
                output, hidden_state = self.decoding_cells[i_layer](current_input, hidden_state, adj_mat)
...
class DCRNNModel(BaseModel):
    def __init__(self, batch_size, enc_input_dim, dec_input_dim, max_diffusion_step, num_nodes,
    ...
    def forward(self, source, target, adj_mat, teacher_forcing_ratio):
        ...
        context, _ = self.encoder(source, init_hidden_state, adj_mat)  # (num_layers, batch, outdim)

        outputs = self.decoder(target, context, adj_mat, teacher_forcing_ratio=teacher_forcing_ratio)
