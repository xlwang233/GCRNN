...
class DiffusionGraphConv(BaseModel):
    def __init__(self, input_dim, hid_dim, num_nodes, max_diffusion_step, output_dim, bias_start=0.0):
    ...
    def forward(self, inputs, state, adj_mat, output_size, bias_start=0.0):
        """
        Diffusion Graph convolution with graph matrix
        :param inputs: should be a dict with the form {graph_id: one batch data}
        :param state:
        :param adj_mat: the SciPy sparse adjacency matrix for graph convolution.
        :param output_size:
        :param bias_start:
        :return:
        """
        ...
        if self._max_diffusion_step == 0:
            ...
            x1 = torch.sparse.mm(adj_mat, x0)
            x = self._concat(x, x1)
            for k in range(2, self._max_diffusion_step + 1):
                x2 = 2 * torch.sparse.mm(adj_mat, x1) - x0
                x = self._concat(x, x2)
                x1, x0 = x2, x1
...
class DCGRUCell(BaseModel):
    ...
    def __init__(self, input_dim, num_units, max_diffusion_step, num_nodes,
        ...
        # self._supports = list()
        # for i in range(adj_mats.shape[0]):  # i from 0 to 951
        #     support = utils.calculate_scaled_laplacian(adj_mats[i], lambda_max=None)  # scipy coo matrix
        #     support = self._build_sparse_matrix(support).cuda()  # to PyTorch sparse tensor
    ...
    def forward(self, inputs, state, adj_mat):
        """
        ...
        if self._use_gc_for_ru:
            ...
            fn = self._fc
        value = torch.sigmoid(fn(inputs, state, adj_mat, output_size, bias_start=1.0))
        value = torch.reshape(value, (-1, self._num_nodes, output_size))
        r, u = torch.split(value, split_size_or_sections=int(output_size/2), dim=-1)
        r = torch.reshape(r, (-1, self._num_nodes * self._num_units))
        u = torch.reshape(u, (-1, self._num_nodes * self._num_units))
        c = self.dconv_candidate(inputs, r*state, adj_mat, self._num_units)  # batch_size, self._num_nodes * output_size
